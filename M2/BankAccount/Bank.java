...
public class Bank {
    ...
    private int search(String accountNumberIn) {
        for (int i = 0; i <= list.size() - 1; i++) {
            ...
            String tempNumber = tempAccount.getAccountNumber(); // get account number if(tempNumber.equals(accountNumberIn)) // if this is the account we are looking for {
    ...
    public boolean addAccount(String accountNumberIn, String nameIn) {
        ...
        {
            list.add(new BankAccount(accountNumberIn, nameIn)); // add new account return true;
        }
        return false;
    }
    ...
    public boolean withdrawMoney(String accountNumberIn, double amountIn) {
            ...
            public boolean removeAccount(String accountNumberIn) {
                int index = search(accountNumberIn); // find index of account if(index != -999) // if account exists account
                {
                    list.remove(index);
